...
const PROVIDER_CONFIGS: Record<RestSttProvider, ProviderConfigFactory> = {
    sarvam: (apiKey, region, languageKey) => {
        const lang = (languageKey && languageKey !== 'auto') ? (RECOGNITION_LANGUAGES[languageKey]?.bcp47 || RECOGNITION_LANGUAGES[languageKey]?.iso639) : undefined;
        return {
...
export class RestSTT extends EventEmitter {
    ...
    /**
     * Set dynamic vocabulary / prompt hint for whisper models
     */
    public setVocabularyPrompt(prompt: string): void {
        this.vocabularyPrompt = (prompt || '').trim();
    }
    ...
    public stop(): void {
        ...
        // Flush remaining audio before disabling active state
        this.flushAndUpload();
        this.isActive = false;
    }
