...
describe('AIProvidersSettings credential synchronization', () => {
  ...
  test('Codex active-model and fast-mode options require enabled config AND signed-in OAuth', () => {
    ...
    assert.match(source, /hasStoredKey\.groq \|\| hasStoredKey\.MeetFloo \|\| \(codexCliConfig\.enabled && codexOauthStatus\.signedIn\)/, 'fast mode should not be enabled by a signed-out Codex flag');
  });
