...
// `!app.isPackaged` AND `process.env.MEETFLOO_DEV_BYPASS_SCREEN_TCC === '1'`.
...
describe("B5: dev-mode TCC bypass is opt-in (MEETFLOO_DEV_BYPASS_SCREEN_TCC=1) only", () => {
  it("isDevTccBypassEnabled() helper exists and checks BOTH !app.isPackaged AND the env flag", () => {
    ...
    assert.ok(
      /process\.env\.MEETFLOO_DEV_BYPASS_SCREEN_TCC/.test(body),
      "BUG: isDevTccBypassEnabled() no longer checks process.env.MEETFLOO_DEV_BYPASS_SCREEN_TCC. " +
    ...
    assert.ok(
      /!\s*app\.isPackaged[\s\S]*&&[\s\S]*MEETFLOO_DEV_BYPASS_SCREEN_TCC/.test(body) ||
      /MEETFLOO_DEV_BYPASS_SCREEN_TCC[\s\S]*&&[\s\S]*!\s*app\.isPackaged/.test(body),
  ...
  it("env var MEETFLOO_DEV_BYPASS_SCREEN_TCC is documented somewhere in main.ts", () => {
    ...
    // escape hatch.
    const occurrences = (main.match(/MEETFLOO_DEV_BYPASS_SCREEN_TCC/g) || []).length;
    assert.ok(
      occurrences >= 1,
      "BUG: env var MEETFLOO_DEV_BYPASS_SCREEN_TCC is no longer referenced in main.ts. " +
  ...
  it("documentation block near isDevTccBypassEnabled mentions both 'granted' and the env-var name", () => {
    ...
    assert.ok(
      /MEETFLOO_DEV_BYPASS_SCREEN_TCC/.test(docWindow),
      "BUG: the doc block above isDevTccBypassEnabled no longer mentions the env-var name " +
      "MEETFLOO_DEV_BYPASS_SCREEN_TCC. Devs reading the helper must be told exactly which " +
