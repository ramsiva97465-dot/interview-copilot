...
// comment claimed it was a no-op in shipped apps, but MEETFLOO_E2E gates only
...
test('e2eInvoke is exposed only behind the MEETFLOO_E2E gate', () => {
  ...
  assert.ok(
    /process\.env\.MEETFLOO_E2E === '1'/.test(before),
    'e2eInvoke must sit inside the MEETFLOO_E2E conditional spread — an unconditional passthrough reaches every production channel (F-117)'
  );
});
