...
// channel failures.
//
// Earlier code (MeetFlooInterface.tsx:929-940 pre-fix) had:
//
...
describe('B1: mic-channel audio-capture-failed must surface in the renderer', () => {
  it('contains exactly one onAudioCaptureFailed subscription', () => {
    ...
    // different file — within MeetFlooInterface.tsx itself there should
    // be a single call site.
    assert.equal(
      subscriptions.length,
      1,
      `BUG: expected exactly 1 onAudioCaptureFailed subscription in MeetFlooInterface.tsx, found ${subscriptions.length}. ` +
