...
export class AppState {
  ...
  private setupAutoUpdater(): void {
    ...
    // Explicitly configure feed URL to target published GitHub repository
    try {
      autoUpdater.setFeedURL({
        provider: 'github',
        owner: 'ramsiva97465-dot',
        repo: 'interview-copilot',
        releaseType: 'release'
      });
      console.log('[AutoUpdater] Set feed URL: ramsiva97465-dot/interview-copilot');
    } catch (feedErr) {
      console.warn('[AutoUpdater] Could not set feed URL:', feedErr);
    }
    ...
    // Check for updates after 10s delay, and repeat periodically every 30 minutes.
    const runAutoUpdateCheck = async () => {
      try {
        if (!app.isPackaged) {
          console.log("[AutoUpdater] Dev mode: Checking updates via GitHub API...");
          await this.checkForUpdatesManual();
        } else {
          console.log("[AutoUpdater] Checking for updates via electron-updater...");
          try {
            const checkResult = await autoUpdater.checkForUpdatesAndNotify();
            if (!checkResult) {
              console.log("[AutoUpdater] electron-updater returned no result, falling back to GitHub API check...");
              await this.checkForUpdatesManual();
            }
          } catch (autoErr) {
            console.warn("[AutoUpdater] electron-updater check failed, falling back to GitHub API check:", autoErr);
            await this.checkForUpdatesManual();
          }
        }
      } catch (err) {
        console.error("[AutoUpdater] Periodic update check encountered error:", err);
      }
    };

    setTimeout(() => {
      runAutoUpdateCheck();
      setInterval(runAutoUpdateCheck, 30 * 60 * 1000);
    }, 10000);
  }
  ...
  private createSTTProvider(speaker: 'interviewer' | 'user'): STTProvider | null {
    ...
    if (sttProvider === 'MeetFloo') {
      ...
      if (apiKey) {
        ...
        if (sarvamFallbackKey) {
          console.log(`[Main] Falling back to backend Sarvam STT for ${speaker}`);
    ...
    } else if (sttProvider === 'groq' || sttProvider === 'azure' || sttProvider === 'ibmwatson' || sttProvider === 'sarvam') {
      ...
      if (sttProvider === 'groq') {
        ...
        apiKey = CredentialsManager.getInstance().getSarvamApiKey() || (process.env.SARVAM_API_KEY || '').trim();
      }
  ...
  private startCaptureChannels(context: string): CaptureChannelStartResult {
    ...
    try {
      ...
      this.sendAudioCaptureFailed({
        channel: 'system',
        message: `System audio loopback inactive (native engine not compiled). Meeting continuing seamlessly with Microphone + Sarvam STT.`,
        attempt: 0,
        maxAttempts: 0,
        terminal: true,
        stuck: false,
  ...
  private async startMeetingTransition(metadata?: any): Promise<void> {
    ...
    // ★ INTERVIEW MODEL FAST AUTO-DETECT & GUARD:
    // When entering the copilot session, ensure the active model is ultra-fast and
    // capable of live dialogue (<1.5s TTFT). If the model was left on a high-latency
    // Pro reasoning model (5-10s TTFT) or has no API key, auto-switch immediately to
    // an interview-ready model (Flash / Sarvam) so the candidate never experiences stalls.
    try {
      const llmHelper = this.processingHelper.getLLMHelper();
      if (llmHelper) {
        ...
        if (check.isAdjusted) {
          console.log(`[Main] Meeting Start Guard: Active model "${currentModel}" is not interview-ready (${check.reason}). Auto-switching to fast interview model "${check.recommendedModel}".`);
          const { CredentialsManager } = require('./services/CredentialsManager');
          const cm = CredentialsManager.getInstance();
          cm.setDefaultModel(check.recommendedModel);
          const all = [...(cm.getCurlProviders() || []), ...(cm.getCustomProviders() || [])];
          llmHelper.setModel(check.recommendedModel, all);
          this.sendModelChanged(check.recommendedModel);
        }
      }
    } catch (e) {
      console.warn('[Main] Meeting start interview model guard error:', e);
    }
...
async function initializeApp() {
  ...
  try {
    const { LocalModelDownloadService, createWhisperDownloadProvider, createNemotronDownloadProvider, createEmbeddingDownloadProvider } = require('./services/LocalModelDownloadService');
    ...
    downloadService.registerProvider(createEmbeddingDownloadProvider());
