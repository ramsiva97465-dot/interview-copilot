...
const ENV_KEYS = [
  'MEETFLOO_INTELLIGENCE_TRACE', 'MEETFLOO_DURABLE_MEMORY_WINDOW', 'MEETFLOO_INTELLIGENCE_OS',
  'MEETFLOO_PROFILE_TREE_V2', 'MEETFLOO_CONTEXT_ROUTER_V2', 'MEETFLOO_LIVE_TRANSCRIPT_BRAIN',
  'MEETFLOO_PROMPT_ASSEMBLER_V2', 'MEETFLOO_ANSWER_DIVERSITY_GUARD', 'MEETFLOO_MEETING_MEMORY_V2',
  'MEETFLOO_MEETING_SUMMARY_V3', 'MEETFLOO_MEETING_MODE_AUTODETECT', 'MEETFLOO_FOLLOWUP_DRAFT_V2',
  'MEETFLOO_SPEAKER_LABELS_V1', 'MEETFLOO_MEETING_NOTES_STRUCTURED_OUTPUT',
  'MEETFLOO_MEETING_SUMMARY_LLM_POLISH', 'MEETFLOO_SPEAKER_DIARIZATION_V1',
  'MEETFLOO_GLOBAL_SEARCH_V2', 'MEETFLOO_IN_MEETING_SEARCH_V2', 'MEETFLOO_CONVERSATION_MEMORY_V2',
  'MEETFLOO_LECTURE_INTELLIGENCE_V2', 'MEETFLOO_DIAGRAM_INTELLIGENCE', 'MEETFLOO_HINDSIGHT_MEMORY',
  'MEETFLOO_HINDSIGHT_LIVE_RECALL', 'MEETFLOO_HINDSIGHT_POST_MEETING_RETAIN',
  ...
  'MEETFLOO_DOC_GROUNDED_STRICT_ISOLATION', 'MEETFLOO_CUSTOM_MODE_SOURCE_ENFORCEMENT',
  'MEETFLOO_DOC_GROUNDED_FALSE_REFUSAL_REPAIR', 'MEETFLOO_JIT_FINAL_ANSWER_ENFORCED',
  'MEETFLOO_CONTEXT_OS', 'MEETFLOO_CONTEXT_OS_MANUAL_CHAT', 'MEETFLOO_CONTEXT_OS_WTA',
  'MEETFLOO_CONTEXT_OS_RECAP_FOLLOWUP', 'MEETFLOO_CONTEXT_OS_EVIDENCE_PACK',
  'MEETFLOO_CONTEXT_OS_MEMORY_SAFETY', 'MEETFLOO_CONTEXT_OS_ENFORCE_CAPABILITIES',
  'MEETFLOO_CONTEXT_OS_PROPERTY_VALIDATION', 'MEETFLOO_CONTEXT_OS_MULTI_FAMILY_EVIDENCE',
  // Pre-existing gap closed 2026-07-25 (found while adding MEETFLOO_TURN_IDENTITY_V2
  // below): MEETFLOO_ANSWER_RELEVANCE_GUARD_LIVE was added to intelligenceFlags.ts's
  ...
  'MEETFLOO_CONTEXT_OS_IMPOSSIBLE_STATE_GATE_SHADOW',
  'MEETFLOO_CONTEXT_OS_IMPOSSIBLE_STATE_GATE_ENFORCE_FORBIDDEN',
  // Prompt System v2 (2026-08-01) — default OFF everywhere (including dev/test).
  'MEETFLOO_PROMPT_SYSTEM_V2',
];
...
describe('intelligenceFlags', () => {
  ...
  test('a newly-added flag can be toggled by env independently', () => {
    process.env.MEETFLOO_CONTEXT_ROUTER_V2 = 'on';
    __resetIntelligenceFlagsCache();
  ...
  test('env override accepts on/true/enabled/yes', () => {
    for (const v of ['on', 'true', 'enabled', 'yes', '1']) {
      process.env.MEETFLOO_DURABLE_MEMORY_WINDOW = v;
      __resetIntelligenceFlagsCache();
  ...
  test('env override OFF wins even if default were ON', () => {
    for (const v of ['off', 'false', '0', 'disabled', 'no']) {
      process.env.MEETFLOO_INTELLIGENCE_TRACE = v;
      __resetIntelligenceFlagsCache();
  ...
  test('unknown env value falls through to default OFF', () => {
    process.env.MEETFLOO_INTELLIGENCE_TRACE = 'maybe';
    __resetIntelligenceFlagsCache();
    assert.equal(isIntelligenceTraceEnabled(), false);
  });
