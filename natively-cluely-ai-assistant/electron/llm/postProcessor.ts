...
export function clampResponse(
    ...
    // Step 2: Strip prefixes (labels) and disclaimers
    result = stripPrefixes(result);
    result = stripDisclaimersAndHeaders(result);
...
/**
 * Strip disclaimers and artificial section headers
 */
export function stripDisclaimersAndHeaders(text: string): string {
    if (!text || typeof text !== "string") return "";
    let result = text;

    // Disclaimers to eliminate completely
    const disclaimerPatterns = [
        /^(?:Transcript|TRANSCRIPT):\s*/gm,
        /\b(?:According to the transcript|Based on the transcript)[^.!?\n]*[.,!?:;]?\s*/gi,
        /\b(?:The uploaded document does not contain|The uploaded document doesn'?t mention|The uploaded document doesn'?t contain)[^.!?\n]*[.,!?:;]?\s*/gi,
        /\b(?:This information is not present in the resume|This information is not available in the resume|This is not mentioned in the resume|This is not present in the uploaded document)[^.!?\n]*[.,!?:;]?\s*/gi,
        /\b(?:The retrieved excerpts do not state|The retrieved excerpts don'?t state)[^.!?\n]*[.,!?:;]?\s*/gi,
        /\b(?:As not directly mentioned in the uploaded material|Not directly mentioned in the uploaded material)[^.!?\n]*[.,!?:;]?\s*/gi,
        /\b(?:(?:The|This)\s+)?(?:uploaded\s+material|document|profile|r[ée]sum[ée]|files?)(?:\s+and\s+(?:my\s+)?(?:r[ée]sum[ée]|profile|document|uploaded\s+material))*\s+(?:does\s+not|doesn'?t|do\s+not|don'?t|cannot|can\s+not)\s+(?:cover|contain|mention|state|have|specify)[^.!?\n]*[.,!?:;]?\s*/gi,
        /\b(?:The\s+(?:r[ée]sum[ée]|resume)\s+and\s+profile\s+material\s+do\s+not\s+cover\s+this\.?\s*)/gi,
        /\b(?:From\s+general\s+knowledge,?\s*)/gi,
        /\b(?:Based\s+on\s+general\s+knowledge,?\s*)/gi,
        /^(?:Here is the (?:code|program|python program|solution|query|sql query|answer)):?\s*/gim,
    ];
