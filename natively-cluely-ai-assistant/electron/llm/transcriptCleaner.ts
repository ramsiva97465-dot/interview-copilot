...
function cleanText(text: string): string {
    ...
    result = normalizeTranscriptVocabulary(result);

    return result;
}

/** Dynamic custom vocabulary terms / company names registered from profile, JD, or user settings */
let dynamicCompanyTerms: string[] = [];

export function setDynamicTranscriptVocabulary(terms: string[]): void {
    dynamicCompanyTerms = terms.filter(Boolean).map(t => t.trim());
}

export function normalizeTranscriptVocabulary(text: string): string {
    if (!text) return text;
    let out = text;
    for (const term of dynamicCompanyTerms) {
        if (!term) continue;
