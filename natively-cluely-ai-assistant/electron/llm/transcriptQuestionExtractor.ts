...
// Conversational closings, polite acknowledgements, and wrap-ups that should never be treated as interview questions.
const CONVERSATIONAL_CLOSING = /^(?:(?:thanks?|thank you|thank you so much|thank you very much|thank you for your time|that'?s all(?: for now| from my side)?|that is all|we will get back to you|we'?ll get back to you|we'?ll be in touch|our team will get back to you|have a (?:great|good|nice) day|nice meeting you|good luck|take care|okay|ok|great|alright|cool|perfect|sounds good|done)[\s!.,?-]*)+$/i;
...
export function extractLatestQuestion(
    ...
    for (let i = cleaned.length - 1; i >= 0; i--) {
        ...
        if (GREETING_ONLY.test(text) || GREETING_ONLY.test(original) || CONVERSATIONAL_CLOSING.test(text) || CONVERSATIONAL_CLOSING.test(original)) {
