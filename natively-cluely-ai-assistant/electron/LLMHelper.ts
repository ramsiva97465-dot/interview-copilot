...
import { CredentialsManager, CustomProvider, CurlProvider } from './services/CredentialsManager';
...
export class LLMHelper {
  ...
  private _sarvamClient: OpenAI | null = null
  ...
  private get sarvamClient(): OpenAI | null { return this.isProviderDisabled('sarvam') ? null : this._sarvamClient }
  private set sarvamClient(v: OpenAI | null) { this._sarvamClient = v }
  ...
  constructor(apiKey?: string, useOllama: boolean = false, ollamaModel?: string, ollamaUrl?: string, groqApiKey?: string, openaiApiKey?: string, claudeApiKey?: string, deepseekApiKey?: string, nvidiaNimApiKey?: string) {
    ...
    const sarvamKey = (CredentialsManager.getInstance().getSarvamApiKey() || process.env.SARVAM_API_KEY || '').trim();
    if (sarvamKey) {
      this.sarvamApiKey = sarvamKey;
      this.sarvamClient = new OpenAI({ apiKey: sarvamKey, baseURL: SARVAM_BASE_URL, defaultHeaders: { 'api-subscription-key': sarvamKey } });
      console.log(`[LLMHelper] Sarvam client initialized with model: ${SARVAM_MODEL}`);
    }
  ...
  public setSarvamApiKey(apiKey: string) {
    const trimmed = (apiKey || '').trim();
    if (!trimmed) {
      const envKey = (process.env.SARVAM_API_KEY || '').trim();
      if (envKey) {
        this.sarvamApiKey = envKey;
        this.sarvamClient = new OpenAI({ apiKey: envKey, baseURL: SARVAM_BASE_URL, defaultHeaders: { 'api-subscription-key': envKey } });
        console.log("[LLMHelper] User Sarvam API Key cleared; reverted to backend process.env.SARVAM_API_KEY.");
      } else {
        this.sarvamApiKey = null;
        this.sarvamClient = null;
        console.log("[LLMHelper] Sarvam API Key cleared.");
      }
      return;
    }
    this.sarvamApiKey = trimmed;
    this.sarvamClient = new OpenAI({ apiKey: trimmed, baseURL: SARVAM_BASE_URL, defaultHeaders: { 'api-subscription-key': trimmed } });
    console.log("[LLMHelper] Sarvam API Key updated.");
  }
  ...
  private isSarvamModel(modelId: string): boolean {
    if (!modelId) return false;
    return modelId.toLowerCase().startsWith("sarvam");
  }
  ...
  /**
   * Check if credentials / clients are available for the specified model in-memory (0ms check).
   * Used to fast-path fallback to Sarvam if the user has selected a model without providing a key.
   */
  public hasKeyForModel(modelId: string): boolean {
    if (!modelId) return false;
    if (this.useOllama || modelId.startsWith('ollama-')) return true;
    if (this.activeCurlProvider || this.customProvider) return true;
    if (this.isCodexCliModel(modelId)) return this.isCodexAvailable();
    if (this.isAntigravityModel(modelId)) {
      try {
        const { AntigravityService } = require('./services/AntigravityService');
        return Boolean(AntigravityService.getInstance().getStatus().signedIn);
      } catch {
        return false;
      }
    }
    if (this.isGroqModel(modelId)) return Boolean(this.groqClient && !this._groqLocalDisabled);
    if (this.isOpenAiModel(modelId)) return Boolean(this.openaiClient);
    if (this.isClaudeModel(modelId)) return Boolean(this.claudeClient);
    if (this.isDeepseekModel(modelId)) return Boolean(this.deepseekClient && !this.deepseekPermanentlyDead);
    if (this.isSarvamModel(modelId)) return Boolean(this.sarvamClient);
  ...
  private buildTextSpareRungs(
    ...
    if (!skip.has('sarvam') && this.sarvamClient) {
      const sarvamSystem = this.injectLanguageInstruction(OPENAI_SYSTEM_PROMPT);
      spares.push({
        id: 'sarvam', name: 'Sarvam AI', isLocal: false, priority: prio++,
        open: (sig) => this.streamWithSarvam(userContent, sarvamSystem, SARVAM_MODEL, sig),
      });
    }
    return spares;
  }
...
${IMAGE_TRUST_TRAILER}`;
  ...
  public async generateSuggestion(context: string, lastQuestion: string): Promise<string> {
    ...
    try {
      ...
      const isTechnicalQuestion = /\b(python|javascript|typescript|java|golang|c\+\+|rust|ruby|php|sql|decorators?|fragments?|arrays?|strings?|dicts?|tuples?|lambdas?|generators?|iterators?|gil|concurrency|multithreading|asynchronous|async|await|algorithm|dsa|function|class|oop|recursion|database|cache|redis|api|rest|graphql|git|docker|kubernetes)\b/i.test(lastQuestion || '')
        || /\b(what (is|are|does)|how (does|do|to)|explain|difference between)\b/i.test(lastQuestion || '');
      const docGroundedEnforcementActive = !isTechnicalQuestion && (groundingInfo?.strictDocumentGroundedActive === true
...
Strictly maintain the interview language of the conversation:
- If the interviewer speaks in English, you MUST generate your response ONLY in English. NEVER generate Hindi answers or default to Hindi when the conversation is in English.
- If Tamil or Tanglish (Tamil written in Latin/English letters) is spoken, respond naturally in Tamil or Tanglish as appropriate. Do NOT switch to Hindi unless Hindi was explicitly spoken.
- If the interviewer explicitly speaks in another language (e.g. Spanish, French, German), respond in that exact same language.
- Under all circumstances, if the input language is English or ambiguous, default strictly to English.
You may mix scripts naturally (e.g. technical code stays in English even when the explanation is in another language).
...
[REMINDER] Your entire response MUST be in ${lang} only. Never switch to English.`;
  ...
  public async chatWithGemini(message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, alternateGroqMessage?: string, routeOptions?: StreamRouteOptions, skipModeInjection?: boolean): Promise<string> {
    try {
      ...
      if (this.isSarvamModel(this.currentModelId) && this.sarvamClient) {
        if (!cloudIsMultimodal) {
          return await this.generateWithSarvam(cloudUserContent, openaiSystemPrompt);
        }
      }
      ...
      // 0ms Fallback: If user has no API key for selected model, immediately return Sarvam LLM response
      if (!this.hasKeyForModel(this.currentModelId) && this.sarvamClient && !cloudIsMultimodal) {
        console.log(`[LLMHelper] 0ms Fallback (non-streaming): User has no API key for selected model "${this.currentModelId}" — falling back to backend Sarvam LLM (${SARVAM_MODEL})`);
        return await this.generateWithSarvam(cloudUserContent, openaiSystemPrompt, SARVAM_MODEL);
      }
      ...
      if (this.sarvamClient && !cloudIsMultimodal) {
        providers.push({ name: `Sarvam AI (${SARVAM_MODEL})`, execute: () => this.generateWithSarvam(cloudUserContent, openaiSystemPrompt, SARVAM_MODEL) });
      }
  ...
  /**
   * Non-streaming Sarvam AI generation via the OpenAI-compatible API.
   */
  private async generateWithSarvam(userMessage: string, systemPrompt?: string, modelId?: string): Promise<string> {
    if (this.isLocalOnlyMode) throw new Error("Cloud providers disabled in local-only mode");
    if (!this.sarvamClient) throw new Error("Sarvam client not initialized");
    this.assertOutboundScopes('sarvam' as any, userMessage);

    await this.rateLimiters.sarvam.acquire();

    const model = modelId || (this.isSarvamModel(this.currentModelId) ? this.currentModelId : SARVAM_MODEL);
    ...
    const response = await this.withTimeout(
      this.withRetry(() => this.sarvamClient!.chat.completions.create({
        model,
        messages,
        max_tokens: 4096,
      })),
  ...
  private async * _streamChatInner(
    ...
    // 0ms Fallback: If user has no API key for selected model, immediately fallback to backend Sarvam LLM
    if (!this.hasKeyForModel(this.currentModelId) && this.sarvamClient && !this.isLocalOnlyMode && !(isMultimodal && imagePaths)) {
      const sarvamSystem = systemPromptOverride || OPENAI_SYSTEM_PROMPT;
      const finalSarvamSystem = this.injectLanguageInstruction(sarvamSystem);
      console.log(`[LLMHelper] 0ms Fallback: User has no API key for selected model "${this.currentModelId}" — falling back to backend Sarvam LLM (${SARVAM_MODEL})`);
      yield* this.streamWithSarvam(userContent, finalSarvamSystem, SARVAM_MODEL, abortSignal);
      return;
    }
    ...
    // Sarvam AI (Text-only conversational model)
    if (this.isSarvamModel(this.currentModelId) && this.sarvamClient && !(isMultimodal && imagePaths)) {
      const sarvamSystem = systemPromptOverride || OPENAI_SYSTEM_PROMPT;
      const finalSarvamSystem = this.injectLanguageInstruction(sarvamSystem);
      yield* this.streamWithSarvam(userContent, finalSarvamSystem, undefined, abortSignal);
      return;
    }
    ...
    // 7. Last-resort fallback: Sarvam AI when all other providers failed or are unconfigured
    if (this.sarvamClient && !this.isLocalOnlyMode && !(isMultimodal && imagePaths)) {
      try {
        const sarvamSystem = systemPromptOverride || OPENAI_SYSTEM_PROMPT;
        const finalSarvamSystem = this.injectLanguageInstruction(sarvamSystem);
        console.log(`[LLMHelper] Falling back to backend Sarvam LLM as last resort — currentModelId is ${this.currentModelId}`);
        yield* this.trackCommit(this.streamWithSarvam(userContent, finalSarvamSystem, SARVAM_MODEL, abortSignal), commit);
        return;
      } catch (e: any) {
        if (commit.emitted) {
          console.warn('[LLMHelper] Sarvam last-resort failed AFTER first token — ending stream rather than appending a second answer:', e?.message || e);
          yield LLMHelper.TRUNCATION_SENTINEL;
          return;
        }
        console.warn('[LLMHelper] Sarvam last-resort failed:', e?.message || e);
      }
    }
  ...
  /**
   * Stream response from Sarvam AI (OpenAI-compatible). Text-only conversational model.
   */
  private async * streamWithSarvam(userMessage: string, systemPrompt?: string, modelId?: string, abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    if (this.isLocalOnlyMode) throw new Error("Cloud providers disabled in local-only mode");
    if (!this.sarvamClient) throw new Error("Sarvam client not initialized");
    this.assertOutboundScopes('sarvam' as any, userMessage);
    ...
    let stream: any;
    try {
      stream = await this.sarvamClient.chat.completions.create({
        model,
        messages,
        stream: true,
        temperature: INTERACTIVE_TEMPERATURE,
      ...
      console.error("[LLMHelper] Sarvam streaming error:", err);
      throw err;
    }
  ...
  /**
   * Get the Sarvam client (OpenAI SDK with Sarvam baseURL) for mode-specific LLMs.
   */
  public getSarvamClient(): OpenAI | null {
    return this.sarvamClient;
  }

  /**
   * Check if Sarvam is available.
   */
  public hasSarvam(): boolean {
    return this.sarvamClient !== null;
  }
