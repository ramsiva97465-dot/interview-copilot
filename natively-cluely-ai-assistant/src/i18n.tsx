...
// (`{t('Start MeetFloo')}`) and means untranslated phrases degrade gracefully to
...
const STORAGE_KEY = 'MeetFloo_lang';
...
const RU: Record<string, string> = {
    ...
    'Routes responses through the fastest available provider (Codex fast mode model, Groq, or MeetFloo). Turn off to use your selected model above.':
        'Направляет ответы через самый быстрый доступный провайдер (быстрый режим Codex, Groq или MeetFloo). Выключите, чтобы использовать выбранную выше модель.',
    'Requires Groq, MeetFloo API, or Codex CLI to be configured.':
        'Требуется настроенный Groq, MeetFloo API или Codex CLI.',
    ...
    'Love MeetFloo? Support us by starring the repo.': 'Нравится MeetFloo? Поддержите нас звездой репозиторию.',
    ...
    'MeetFloo is independent source-available software.': 'MeetFloo — независимое ПО с открытым исходным кодом.',
    ...
    'MeetFloo can capture both what you say and what you hear globally. At the top of the Audio Settings, use the Dropdowns to explicitly select your hardware Input (e.g. your physical microphone) and Output capture (what the speakers play). By default, MeetFloo uses the System Default, so audio routing automatically follows your OS preferences.':
    ...
    'MeetFloo supports over 8 different Audio engines to transcribe what you hear and say. From the Audio tab in settings, use the overarching dropdown to switch the active engine.':
    ...
    'Customize how MeetFloo works for you': 'Настройте MeetFloo под себя',
    'MeetFloo is currently detectable by screen-sharing.': 'Сейчас MeetFloo виден при демонстрации экрана.',
    ...
    'Open MeetFloo when you log in': 'Открывать MeetFloo при входе в систему',
    'MeetFloo will open automatically when you log in to your computer.':
        'MeetFloo будет открываться автоматически при входе в систему.',
    'MeetFloo will open automatically when you log in to your computer':
        'MeetFloo будет открываться автоматически при входе в систему',
...
// ES_GENERATED is exported as ES_GENERATED, so we spread ES_GENERATED.
