...
const ModelSelectorWindow = () => {
    ...
    const handleSelectFn = async (modelId: string) => {
        try {
            const res = await window.electronAPI?.setModel(modelId);
            const targetModel = res?.effectiveModel || modelId;
            setCurrentModel(targetModel);
            localStorage.setItem('cached-current-model', targetModel);

            if (res?.adjusted && res?.reason) {
                setAdjustmentNotice(res.reason);
                setTimeout(() => setAdjustmentNotice(null), 4500);
            }
        } catch (err: any) {
        ...
        <div className="w-fit h-fit bg-transparent flex flex-col">
            ...
            >
                <div className="relative z-[1] flex-1 min-h-0 flex flex-col">
                    {adjustmentNotice && (
                        <div className="mb-1 p-1.5 rounded-lg bg-amber-500/15 border border-amber-500/30 text-[10px] text-amber-300 leading-tight shrink-0">
                            ⚡ {adjustmentNotice}
                        </div>
                    )}
                    {isLoading ? (
                        ...
                        <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col gap-0.5">
                            {availableModels.length === 0 ? (
                                ...
                                availableModels.map((model) => {
                                    ...
                                    const isSlow = /pro|opus|o1|o3|reason/i.test(model.id) || /pro|opus|reasoning/i.test(model.name);
                                    return (
                                        <button
                                            key={model.id}
                                        ...
                                        >
                                            <div className="flex items-center gap-1 truncate flex-1 min-w-0">
                                                ...
                                                {isSlow ? (
                                                    <span className="text-[10px] shrink-0 opacity-70" title="Heavy Reasoning (5s+) — slower in live interviews">🐢</span>
                                                ) : (
                                                    <span className="text-[10px] shrink-0 text-emerald-400 opacity-80" title="Interview Ready (<1s)">⚡</span>
                                                )}
                                            </div>
