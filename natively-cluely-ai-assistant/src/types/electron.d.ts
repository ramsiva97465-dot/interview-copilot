...
export interface ElectronAPI {
  ...
  localEmbeddingStartDownload?: (modelId: string) => Promise<{ success: boolean; error?: string; alreadyDownloading?: boolean }>
  localEmbeddingCancelDownload?: (modelId: string) => Promise<{ success: boolean; error?: string }>
  localEmbeddingGetDownloadState?: (modelId?: string) => Promise<any>
  localEmbeddingDeleteModel?: (modelId: string) => Promise<{ success: boolean; error?: string }>
  ...
  testSttConnection: (provider: 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'nvidia_nim' | 'sarvam', apiKey: string, region?: string) => Promise<{ success: boolean; error?: string }>
  ...
  cloudflareTestConnection: () => Promise<{ success: boolean; latencyMs?: number; error?: string }>
  cloudflareFetchMeetings: (userId?: string) => Promise<any[]>
  cloudflareSyncMeeting: (meeting: any) => Promise<boolean>
  cloudflareFetchProfile: (docType: string, userId?: string) => Promise<any | null>
