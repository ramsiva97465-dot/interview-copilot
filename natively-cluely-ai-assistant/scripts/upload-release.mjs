...
// MeetFloo release Google Drive folder, via the existing `rclone` remote
// named `gdrive:`.
//
...
//   name: "MeetFloo 2.8.1 beta 1"  (this may evolve per release)
...
//
// SKIP ESCAPE HATCHES:
//   MEETFLOO_SKIP_RELEASE_UPLOAD=1  → exit 0 silently (local dev / CI dry-runs)
//   MEETFLOO_SKIP_RELEASE_UPLOAD=prompt  → ask before uploading (default in TTY)
//
// Usage:
//   node scripts/upload-release.mjs                       # upload current version
//   MEETFLOO_SKIP_RELEASE_UPLOAD=1 node scripts/upload-release.mjs
//   MEETFLOO_RELEASE_VERSION=2.8.0 node scripts/upload-release.mjs  # explicit override
...
async function main() {
  ...
  if (!/^\d+\.\d+\.\d+(-\w+(\.\d+)?)?$/.test(version)) {
    fail(`could not resolve a valid version (got "${version}"). Set MEETFLOO_RELEASE_VERSION or fix package.json#version.`);
  }
  ...
  //      (a) `productName` may be missing from package.json (live: only `name:"MeetFloo"`)
  //      (b) scripts/afterAllArtifactBuild.cjs hardcodes VOLNAME='MeetFloo' so the
  //          actual filename casing is "MeetFloo-…", not always matching productName
  //      (c) multiple architectures can ship side-by-side (universal / arm64 / x64)
  const productName = pkg.productName || pkg.name || 'MeetFloo';
  const dmgRegexes = [
    new RegExp(`^MeetFloo-${version.replace(/\./g, '\\.')}\\.dmg$`, 'i'),
    new RegExp(`^MeetFloo-${version.replace(/\./g, '\\.')}-arm64\\.dmg$`, 'i'),
    new RegExp(`^MeetFloo-${version.replace(/\./g, '\\.')}-x64\\.dmg$`, 'i'),
  ...
  // 3. Skip if requested
  if (process.env.MEETFLOO_SKIP_RELEASE_UPLOAD === '1') {
    console.log('[upload-release] MEETFLOO_SKIP_RELEASE_UPLOAD=1 — skipping upload.');
    process.exit(0);
  }

  // 4. Confirm in interactive mode (TTY) — auto-upload only if -y / CI / explicit yes
  const autoYes =
    process.env.MEETFLOO_SKIP_RELEASE_UPLOAD !== 'prompt' &&
    (process.env.CI === 'true' || process.env.CI === '1' || !process.stdin.isTTY || process.argv.includes('-y'));
  if (process.env.MEETFLOO_SKIP_RELEASE_UPLOAD !== 'prompt' && !autoYes && process.stdin.isTTY) {
  ...
  //    matches the source — preserves `MeetFloo-2.8.1.dmg` / `-arm64.dmg` exactly,
