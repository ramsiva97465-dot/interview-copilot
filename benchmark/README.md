# MeetFloo summary-model benchmark (2026-09-16/17)

Compares DeepSeek V4.1 Flash (current MeetFloo config and a thinking-on diagnostic) with GPT-5.6 Luna at reasoning effort max, medium, low and none. Every configuration runs through **MeetFloo's real post-meeting summary pipeline**: the production `electron/` meeting-notes V3 code and an unmodified local `MeetFloo-api`.
...
- MeetFloo-api submodule: `0adab92` at start. Unrelated reconciler commits (`a179c37..08b33a6`) landed mid-run; `server.js` doesn't import them.
...
```
transcripts/<ID>.segments.json         (MeetFloo TranscriptSegment[])
  → harness/run-pipeline.cjs            (Electron-as-Node; replicates MeetingPersistence.processAndSaveMeeting V3 branch)
      ...
      LLM seam: real LLMHelper.generateMeetingSummary → generateWithMeetFloo → POST /v1/chat
  → local MeetFloo-api (node --import harness/provider-shim.mjs server.js)
...
```
Keys are read from the existing `.env` files and never printed or written. `MeetFloo-api` runs with production Supabase, telemetry and alerts disconnected, for the child process only.

## Status / open items
- **13 Luna max runs excluded.** The OpenAI account ran out of credits (HTTP 429 "no credits remaining") around 20:40 UTC. Once credits are added, rerun: `orchestrator.mjs --retry-failed --configs luna-max`, then the judge, analysis and report. Estimated cost ≈ $3. **Decision (2026-09-17): finalized as-is.** No OpenAI top-up; the shared OpenRouter key was not used, because MeetFloo-api also uses it for /v1/embed and /v1/rerank.
...
| `transcripts/` | `CORPUS_SPEC.md`; `src/*.txt` authored/caption transcripts; `*.segments.json` MeetFloo segments |
