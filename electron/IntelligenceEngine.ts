                ...
                try {
                    ...
                    if (sani.repaired && !sani.needsFallback) {
                        ...
                        if (documentGroundedCustomModeActive) {
                            ...
                            const orch = this.llmHelper?.getKnowledgeOrchestrator?.();
                            const activeResume = (orch as any)?.activeResume?.structured_data;
                            const candidateName = activeResume?.identity?.name;
                            const cleanQ = (question || extractedQuestion.latestQuestion || lastInterviewerTurn || '').toLowerCase();
                            const isIdentityQ = /\b(your name|who are you|what'?s your name|what is your name|introduce yourself|tell me about yourself)\b/.test(cleanQ);

                            if (isIdentityQ && candidateName) {
                                const role = activeResume?.identity?.title || activeResume?.experience?.[0]?.role || '';
                                fullAnswer = role ? `I'm ${candidateName}, ${role}.` : `I'm ${candidateName}.`;
                            } else {
                                const regenerated = await this.regenerateUsableAnswer({
                                    ...
                                    signal: whatToAnswerCancellationToken.signal,
                                    isSuperseded: isWtaSuperseded,
                                    reason: 'candidate_voice_misfire',
                                });
                                if (regenerated) {
                                    fullAnswer = regenerated;
                                } else if (candidateName) {
                                    fullAnswer = `I'm ${candidateName}.`;
                                } else {
                                    fullAnswer = "I'm ready to discuss my background and how I can help your team.";
                                }
                            }
                        }
                    }
            ...
            if (ASSISTANT_VOICE_ANSWER_TYPES.has(answerPlan.answerType)
                ...
                try {
                    ...
                    if (mis.isMisfire) {
                        ...
                        if (regenerated) {
                            fullAnswer = regenerated;
                        } else if (mis.reason === 'identity') {
                            const orch = this.llmHelper?.getKnowledgeOrchestrator?.();
                            const activeResume = (orch as any)?.activeResume?.structured_data;
                            const candidateName = activeResume?.identity?.name;
                            const role = activeResume?.identity?.title || activeResume?.experience?.[0]?.role || '';
                            fullAnswer = candidateName ? (role ? `I'm ${candidateName}, ${role}.` : `I'm ${candidateName}.`) : "I'm ready to discuss my background and how I can help your team.";
                        } else {
