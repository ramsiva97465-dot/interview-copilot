...
interface ElectronAPI {
  ...
  testLlmConnection: (
    provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'nvidia_nim' | 'sarvam',
    apiKey?: string,
  ) => Promise<{ success: boolean; error?: string }>;
  ...
  setSarvamApiKey: (apiKey: string) => Promise<{ success: boolean; error?: string }>;
  ...
  setModel: (modelId: string) => Promise<{ success: boolean; error?: string; effectiveModel?: string; adjusted?: boolean; reason?: string; originalModel?: string }>;
  setDefaultModel: (modelId: string) => Promise<{ success: boolean; error?: string; effectiveModel?: string; adjusted?: boolean; reason?: string; originalModel?: string }>;
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  testLlmConnection: (provider: 'gemini' | 'groq' | 'openai' | 'claude' | 'deepseek' | 'nvidia_nim' | 'openrouter' | 'fluxion' | 'sarvam', apiKey: string) =>
  ...
  setSarvamApiKey: (apiKey: string) => ipcRenderer.invoke('set-sarvam-api-key', apiKey),
  ...
  // HuggingFace Local Embedding Model download & management
  localEmbeddingStartDownload: (modelId: string) =>
    ipcRenderer.invoke('local-embedding-start-download', modelId),
  localEmbeddingCancelDownload: (modelId: string) =>
    ipcRenderer.invoke('local-embedding-cancel-download', modelId),
  localEmbeddingGetDownloadState: (modelId?: string) =>
    ipcRenderer.invoke('local-embedding-get-download-state', modelId),
  localEmbeddingDeleteModel: (modelId: string) =>
    ipcRenderer.invoke('local-embedding-delete-model', modelId),
  onLocalEmbeddingDownloadState: (
    cb: (data: {
      provider: string;
      modelId: string;
      status: 'downloading' | 'verifying' | 'complete' | 'error' | 'cancelled' | 'interrupted';
      progress: number;
      error?: string;
    }) => void,
  ) => {
    const listener = (_: any, data: any) => cb(data);
    ipcRenderer.on('local-model:embedding:download-state', listener);
    return () => ipcRenderer.removeListener('local-model:embedding:download-state', listener);
  },
  ...
  // Cloudflare D1 Cloud Database API
  cloudflareTestConnection: () => ipcRenderer.invoke('cloudflare-test-connection'),
  cloudflareFetchMeetings: (userId?: string) => ipcRenderer.invoke('cloudflare-fetch-meetings', userId),
  cloudflareSyncMeeting: (meeting: any) => ipcRenderer.invoke('cloudflare-sync-meeting', meeting),
  cloudflareFetchProfile: (docType: string, userId?: string) => ipcRenderer.invoke('cloudflare-fetch-profile', docType, userId),
