...
export function parseJudgeVerdict(raw: string | null | undefined, candidateText: string): JudgeVerdict | null {
    ...
    const isClosingOrBackchannel = /^(?:(?:yeah|yes|yep|yup|ya|mm-?hm+|mhm+|uh-?huh|ok(?:ay)?|right|sure|cool|got it|i see|nice|great|perfect|exactly|interesting|makes sense|sounds good|true|correct|wow|oh|ah|hm+|haha+|alright|of course|fair enough|no problem|totally|absolutely|definitely|indeed|good|fine|thanks?|thank you|thank you so much|thank you very much|thank you for your time|that'?s all|that is all|that'?s all for now|that'?s all from my side|we will get back to you|we'?ll get back to you|we'?ll be in touch|have a (?:great|good|nice) day|nice meeting you|good luck|take care|done)[\s,.!?-]*)+$/i.test(candidateText.trim());
    let action: JudgeAction = rawAction === 'answer' || rawAction === 'offer'
        ? 'answer'                                   // 'offer' is retired: doubt answers
        : rawAction === 'silent'
            ? 'silent'
            : (!isAsk || !directed ? 'silent' : 'answer');
    let finalAnswerability = answerability;
    let finalIsAsk = isAsk;
    if (isClosingOrBackchannel) {
        action = 'silent';
        finalAnswerability = 0.0;
        finalIsAsk = false;
    }
    return { isAsk: finalIsAsk, directedAtUser: directed, complete, act, answerability: finalAnswerability, questionText, action };
}
