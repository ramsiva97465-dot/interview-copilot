            ...
            for (const leg of legacyCandidates) {
                ...
                if (fs.existsSync(legDb)) {
                    try {
                        ...
                        if (fs.existsSync(legSettings) && !fs.existsSync(targetSettings)) {
                            fs.copyFileSync(legSettings, targetSettings);
                        }
                        break;
                    } catch (migErr) {
                        console.warn('[DatabaseManager] Migration failed (non-fatal):', migErr);
                    }
                }
            }
        }
    ...
    public setActiveMode(id: string | null): void {
        ...
        try {
            const txn = this.db.transaction(() => {
                ...
                if (id) {
                    let result = this.db!.prepare('UPDATE modes SET is_active = 1 WHERE id = ?').run(id);
                    if (result.changes === 0) {
                        result = this.db!.prepare("UPDATE modes SET is_active = 1 WHERE template_type = ? AND id != '__profile_okf__'").run(id);
                    }
                    if (result.changes === 0) {
                        console.warn(`[DatabaseManager] setActiveMode: no mode found with id or template_type "${id}" — active mode cleared`);
                    }
                }
            });
            txn();
