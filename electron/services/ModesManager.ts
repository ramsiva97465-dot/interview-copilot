...
export class ModesManager {
    ...
    private resolveMode(pinnedModeId?: string): Mode | null {
        if (pinnedModeId) {
            const pinned = this.getModes().find(m => m.id === pinnedModeId || m.templateType === pinnedModeId);
    ...
    public setActiveMode(id: string | null): void {
        ...
        let resolvedId = id;
        if (id) {
            const match = this.getModes().find(m => m.id === id || m.templateType === id);
            if (match) {
                resolvedId = match.id;
            }
        }
        DatabaseManager.getInstance().setActiveMode(resolvedId);
        this.invalidateActiveModeCache();
    }
