// CredentialsManager.revertMeetFlooAutoDefaults — the undo for a key the server
// turns out to refuse.
//
...
//      that was already stored leaves the snapshot reading 'MeetFloo' too, so
//      restoring the snapshot would restore the broken state.
//   2. It touches ONLY 'MeetFloo'. A model the user picked deliberately is not
...
test('a refused key un-parks the default model from MeetFloo', () => {
  const env = makeEnv();
  const cm = freshManager(env);
  cm.setDefaultModel('MeetFloo');

  const changed = cm.revertMeetFlooAutoDefaults('MeetFloo key refused by server');
...
test('reverting works when the model was ALREADY MeetFloo before the save', () => {
  // The re-save case. A snapshot-based revert would restore 'MeetFloo' here and
  // leave the user exactly as broken as before.
  const env = makeEnv();
  const cm = freshManager(env);
  cm.setMeetFlooApiKey('MeetFloo_sk_whatever');
  assert.equal(cm.getDefaultModel(), 'MeetFloo', 'precondition: the key save auto-promoted the model');

  cm.revertMeetFlooAutoDefaults('MeetFloo key refused by server');
...
test('a MeetFloo STT provider is reverted and reported (the caller must rebuild the pipeline)', () => {
  const env = makeEnv();
  const cm = freshManager(env);
  cm.setSttProvider('MeetFloo');
...
test('a non-MeetFloo STT provider is left alone', () => {
  const env = makeEnv();
  const cm = freshManager(env);
...
test('set-MeetFloo-api-key undoes the auto-promotion when the key is REFUSED', () => {
  const block = sliceSafeHandleBlock(HANDLER_BLOCK, 'set-MeetFloo-api-key');
  ...
  assert.match(
    block,
    /revertMeetFlooAutoDefaults/,
    'a refused key leaves the user parked on the MeetFloo endpoint unless the promotion is undone',
  );
});

test('set-MeetFloo-api-key reports a refusal instead of returning unconditional success', () => {
  const block = sliceSafeHandleBlock(HANDLER_BLOCK, 'set-MeetFloo-api-key');
  assert.match(
    block,
