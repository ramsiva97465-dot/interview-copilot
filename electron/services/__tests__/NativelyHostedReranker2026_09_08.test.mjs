/**
 * MeetFloo as a hosted reranker provider — the managed rerank-2.5-lite served by
 * MeetFloo-api's POST /v1/rerank.
 *
 * WHY THIS FILE EXISTS
 *
 * Every other hosted provider here is bring-your-own-key. This one is not: it
 * runs on the MeetFloo API key the user already has and bills against their
 ...
 *
 * It is a table entry rather than a second client because MeetFloo-api speaks
...
describe('the MeetFloo descriptor', () => {
  test('is registered and resolvable by id', () => {
    ...
    const p = hostedRerankProvider('MeetFloo');
    assert.ok(p, 'MeetFloo must resolve');
    assert.equal(p.id, 'MeetFloo');
    assert.equal(HOSTED_RERANK_PROVIDERS.MeetFloo, p);
  });

  test('points at the MeetFloo API, not a third party', () => {
    const p = hostedRerankProvider('MeetFloo');
    assert.match(p.baseUrl, /\/v1$/);
    assert.match(p.baseUrl, /MeetFloo|127\.0\.0\.1|localhost/);
  });
  ...
  test('offers exactly the one model the server serves', () => {
    ...
    assert.equal(defaultHostedModel('MeetFloo'), 'rerank-2.5-lite');
  });

  test('an unknown id is still rejected', () => {
    assert.equal(hostedRerankProvider('MeetFlooy'), null);
...
describe('model resolution', () => {
  test('an unset model falls back to the managed one, not to ineligibility', () => {
    ...
    assert.equal(readHostedModel({ provider: 'MeetFloo' }), 'rerank-2.5-lite');
    assert.equal(readHostedModel({ provider: 'MeetFloo', MeetFlooModel: 'rerank-2.5-lite' }), 'rerank-2.5-lite');
  });
  ...
  test('it does not read the OpenRouter or Jina model fields', () => {
    ...
    const m = readHostedModel({ provider: 'MeetFloo', openrouterModel: 'x/y', jinaModel: 'jina-reranker-v3' });
...
describe('eligibility', () => {
  ...
  test('MeetFloo is an eligible hosted provider', () => {
    // The gate used to be `provider !== 'openrouter' && provider !== 'jina'`.
    // Without MeetFloo added, every managed rerank was refused as
    // 'provider-not-selected'.
    assert.deepEqual(evaluateHostedEligibility({ ...base, provider: 'MeetFloo' }), { eligible: true });
  });
  ...
  test('being the managed tier is NOT an exemption from the privacy scope', () => {
    ...
    // does not leave this machine meant MeetFloo too.
    assert.deepEqual(
      evaluateHostedEligibility({ ...base, provider: 'MeetFloo', referenceFilesScopeAllowed: false }),
      { eligible: false, reason: 'reference-files-scope-denied' },
    );
    assert.deepEqual(
      evaluateHostedEligibility({ ...base, provider: 'MeetFloo', localOnly: true }),
      { eligible: false, reason: 'local-only-mode' },
    );
  });

  test('no MeetFloo key means no managed rerank', () => {
    assert.deepEqual(
      evaluateHostedEligibility({ ...base, provider: 'MeetFloo', hasApiKey: false }),
      { eligible: false, reason: 'no-api-key' },
    );
  });

  test('the ineligibility copy does not name OpenRouter at a MeetFloo user', () => {
...
describe('the wire, through the shared client', () => {
  ...
  test('sends the contract MeetFloo-api /v1/rerank actually implements', async () => {
    const { calls, impl } = recordingFetch({
      model: 'rerank-2.5-lite',
      tokens: 47,
      results: [{ index: 1, relevance_score: 0.79 }, { index: 0, relevance_score: 0.43 }],
    });
    const descriptor = hostedRerankProvider('MeetFloo');
    const r = new OpenRouterReranker({
      baseUrl: descriptor.baseUrl,
      providerId: descriptor.id,
      getApiKey: () => 'MeetFloo_sk_test',
      getModel: () => defaultHostedModel('MeetFloo'),
      fetchImpl: impl,
    });
    ...
    // Bearer, not x-MeetFloo-key: the shared client sends only Authorization,
    ...
    assert.equal(calls[0].init.headers.Authorization, 'Bearer MeetFloo_sk_test');
