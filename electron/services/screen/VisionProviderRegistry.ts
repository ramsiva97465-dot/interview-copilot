...
export function buildVisionProviders(inputs: VisionProviderBuildInputs): VisionProviderConfig[] {
  ...
  if (cloudAllowed) {
    // 1. BYOP (Bring Your Own Provider) — prioritize user's own configured API keys first
...
function sarvam(creds: CredentialsManager, _inputs: VisionProviderBuildInputs): VisionProviderConfig {
  const apiKey = creds.getSarvamApiKey() || (process.env.SARVAM_API_KEY || '').trim();
  return {
    id: 'sarvam',
    displayName: 'Sarvam AI',
    modelId: 'sarvam-105b-conversations',
    isLocal: false,
    isConfigured: !!apiKey,
    supportsVision: false,
    scopeAllowsScreenshots: true,
    hint: 'generic',
    invoke: async (p) => callLLMHelperVision('sarvam', p),
  };
}
