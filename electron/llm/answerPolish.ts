...
/**
 * Strip model reasoning blocks, scratchpad drafts, and closing tags like </think> or /assistant></think>.
 * When a model outputs its deliberation followed by </think> before the real answer,
 * discard the scratchpad so only the final answer remains.
 */
export function stripReasoningAndScratchpad(text: string): string {
  if (!text) return text;
...
export function cleanAnswerArtifacts(text: string): string {
  if (!text) return text;
  // Strip any reasoning / scratchpad leak from thinking models (Sarvam 105B / Qwen / DeepSeek)
  text = stripReasoningAndScratchpad(text);
