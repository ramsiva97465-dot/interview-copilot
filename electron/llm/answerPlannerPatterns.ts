...
export // ── ACTUAL SOURCE CODE evidence requests (release 2026-06-06b) ──
  // "a snippet you used to build MeetFloo", "repo-verifiable code", "actual code
  ...
  const SOURCE_CODE_EVIDENCE_PATTERNS = [
    // "actual/real/exact code ... of MEETFLOO / your repo / the codebase / github" —
    ...
    /\b(actual|real|exact|repo[- ]?verifiable|github[- ]?verifiable)\s+(code|snippet|implementation|function|source)\b.{0,50}\b(MeetFloo|nativley|your (repo|codebase|source|project)|the (repo|codebase|source|project)|github|gitlab)\b/i,
    /\b(MeetFloo|nativley|your (repo|codebase|source|project)|the (repo|codebase)|github)\b.{0,50}\b(actual|real|exact|repo[- ]?verifiable)\s+(code|snippet|implementation|function|source)\b/i,
    /\b(snippet|code|function|implementation|file)\b.{0,40}\b(you (used|wrote|built|made)|from (your|the) (codebase|repo|repository|source|github|project)|to (build|built) MeetFloo)\b/i,
    // "what does your actual <X> code look like", "show me your <X> code", "your
    // real code for <X>" — asking about MEETFLOO's own implementation. A source-
    ...
    // MeetFloo snippet. Every added lead-in is an explicit speech act, so the
    // bare-stub-comment case stays unmatched.
    /\b(what does|what'?s|whats|show me|tell me about|can i see|could i see|can you show|could you show|let me see|let me look at|i want to see|i'?d like to see|i wanna see)\s+(your|the MeetFloo|MeetFloo'?s)\s+(actual\s+|real\s+)?[\w ]*\bcode\b\s*(look|is|for|of)?/i,
    ...
    // "paste/show/give a snippet from the MeetFloo repo/codebase/source"
    /\b(paste|show|give|share|pull)\b.{0,30}\b(snippet|code|function|file)\b.{0,30}\b(from (the )?(MeetFloo|nativley) (repo|codebase|source|project)|from (your|the) (repo|codebase|github))\b/i,
    /\bsnippet from (the )?(MeetFloo|nativley|your|the) (repo|codebase|source|project|github)\b/i,
    ...
    /\b(show|give|write|share)\b.{0,40}\b(code|snippet)\b.{0,40}\b(you (used|wrote)|to (build|built)|from MeetFloo|actual|real|repo|github)\b/i,
    /\bdemo code of a snippet you have used\b/i,
    /\b(exact|actual) code from (file|the file|your)\b/i,
    // "write/give a demo snippet FOR MEETFLOO" / "demo code for MeetFloo" — a request
    // for code OF the loaded project (even with a write-verb, the "for MeetFloo"
    ...
    /\b(write|give|show|share|make)\b.{0,30}\b(demo |sample |example )?(code|snippet)\b.{0,20}\b(for|of|from)\s+(MeetFloo|nativley|your project|the project)\b/i,
    /\b(demo|sample|example)\s+(code|snippet)\s+(for|of|from)\s+(MeetFloo|nativley|the MeetFloo)\b/i,
...
export // ── PRODUCT / PROJECT "what is it" questions (release 2026-06-06b) ──
  // "what kind of app is MeetFloo?", "how's its backend?", "what do you think about
  // MeetFloo?", "what tech does it use?". Grounded in loaded project metadata.
  ...
  const PRODUCT_ABOUT_PATTERNS = [
    ...
    /\bhow(?:'?s| is| does)\s+(MeetFloo|nativley|nativly|it|the (app|product|backend|architecture|frontend|stack))\b/i,
    /\bwhat\s+(do you think about|about)\s+(MeetFloo|nativley|nativly)\b/i,
    /\bwhat (tech|technolog|stack|languages?|framework)\w*\s+(does|do)\s+(MeetFloo|nativley|it|this)\b/i,
    /\bis (MeetFloo|nativley|it|this)\s+(local|cloud|open[- ]?source|source[- ]?available|privacy|low[- ]?distraction|on[- ]?device|transparent|accessib)\w*/i,
    /\b(MeetFloo|nativley|nativly)'?s\s+(backend|architecture|stack|frontend|core)\b/i,
    ...
    /\b(does|do)\s+(it|this|MeetFloo|nativley)\s+(process|run|store|work|use|have|support|need)\b/i,
    /\b(what|which) part (of (MeetFloo|nativley|it|the app))?\s*(uses|is in|runs|handles|does)\b|\b(does|do) (it|MeetFloo) (use|have) (a )?(backend|server|database|ollama|rust|electron|local)\b/i,
    ...
    /\bwhat (does|do) (MeetFloo|nativley|it) use\b|\bwhat'?s (MeetFloo|nativley|it) (built|made|written) (with|in)\b/i,
    // Architecture / build-stack questions ABOUT the product: "what is MeetFloo built
    // with", "what is it made using", "what are the technologies behind MeetFloo",
    // "what is the architecture of MeetFloo", "how did you build MeetFloo" (release
    ...
    /\bwhat (is|'?s|are) (the )?(tech ?(stack)?|technolog\w*|stack|architecture|framework\w*)\s+(of\s+|behind\s+|powering\s+)?(MeetFloo|nativley|it|this|the (app|product|project))\b/i,
    /\b(MeetFloo|nativley|nativly|it|this)\s+(is\s+)?(built|made|written|developed|created|powered)\s+(with|using|in|on)\b/i,
    /\bwhat (is|'?s) (it|MeetFloo|nativley)\s+(made|built|written|developed)\s+(of|with|using|in)\b/i,
    /\b(what is|whats|describe) (the )?architecture (of )?(MeetFloo|nativley|it|this|the (app|product|project))\b/i,
    /\bhow (did|do|was) (you|MeetFloo|it|this)\s+(build|built|develop\w*|architect\w*|design\w*)\s+(MeetFloo|it|this|the (app|product))\b/i,
    /\bhow (is|was) (MeetFloo|nativley|it|this) (built|made|developed|architected|designed)\b/i,
    // "how (do you make|to make) it low-distraction / privacy-first / local" — a
    // product-design question about MeetFloo, grounded in metadata (1000-q
    // benchmark 2026-06-06b). NOT a stealth ask (no evasion/interview object).
    /\bhow (do you |to )?(make|keep|design)\s+(it|MeetFloo|this)\s+(low[- ]?distraction|privacy[- ]?first|private|transparent|accessible|local|on[- ]?device|minimal)\b/i,
    /\b(low[- ]?distraction|privacy[- ]?first)\b.{0,30}\b(mode|design|approach|first)\b|\bkeep (it|MeetFloo|this) (low[- ]?distraction|privacy)/i,
    ...
    /\bhow (to|do i|should i) disclose (it|MeetFloo|this|using it)\b|\bdisclose (it|MeetFloo|this) (in|during|to)\b/i,
    /\bmake (it|MeetFloo|this) accessible\b|\baccessible (without|but not) (being )?distract\w*/i,
  ];
...
export const JD_FIT_PATTERNS = [
  ...
  /\b(why|how) (is|does) (MeetFloo|this project|that project|your project|it)\b.*\b(relevant|prove|matter|fit|qualify|show)\b.*\b(analyst|data|role|job)?/i,
...
export const PROJECT_PATTERNS = [
  ...
  // "Tell me about MeetFloo", "explain MeetFloo", "what is MeetFloo", "talk about
  // MeetFloo" — direct asks about a named project (benchmark 2026-06-05). The
  // known project entity is resolved at runtime; here we recognise the intent.
  /\b(tell me about|talk about|explain|describe|walk me through|what(?:'s| is)?)\s+MeetFloo\b/i,
...
export const EXPERIENCE_PATTERNS = [
  ...
  /\bwhat do you think (about|of)\s+(?!the\b|this\b|that\b|your\b|my\b|it\b|all\b|everything\b|us\b|them\b|MeetFloo|nativley|the (role|job|company|position|team))[a-z][\w-]{3,}\b/i,
