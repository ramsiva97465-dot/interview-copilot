export type LLMProviderId = 'MeetFloo' | 'groq' | 'codex' | 'gemini_flash' | 'gemini_pro' | 'openai' | 'claude' | 'deepseek' | 'ollama';
...
export function routeLLMProviders(options: ProviderRouteOptions): ProviderAttempt[] {
    ...
    const MeetFloo: ProviderSpec = {
        provider: 'MeetFloo',
        name: 'MeetFloo API',
        model: models.MeetFloo,
        available: Boolean(availability.hasMeetFloo),
    ...
    const orderedSpecs: ProviderSpec[] = options.multimodal
        ? [MeetFloo, codex, openai, geminiFlash, claude, geminiPro, groq]
        : [MeetFloo, groq, codex, geminiFlash, geminiPro, openai, claude, deepseek];
...
export class ProviderRouter {
    ...
    constructor(circuitConfig?: Partial<CircuitBreakerConfig>) {
        ...
        ['gemini', 'groq', 'openai', 'claude', 'deepseek', 'MeetFloo', 'codex'].forEach(provider => {
    ...
    selectProvider(policy: RoutingPolicy): ProviderChoice {
        ...
        const availableProviders = this.filterHealthyProviders(
            ['gemini', 'groq', 'openai', 'claude', 'deepseek', 'MeetFloo', 'codex'],
            health
        );
