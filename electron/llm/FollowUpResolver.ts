...
export function resolveFollowUp(ctx: FollowUpContext): ResolvedFollowUp {
  ...
  // Long, self-contained questions are not bare follow-ups, unless they explicitly carry demonstrative project/turn anchors.
  const wordCount = q.split(/\s+/).filter(Boolean).length;
  const hasProjectAnchor = /\b(?:that project|this project|the project|in it|for it|there|you mentioned|why did you (?:choose|pick|use|select)|why\s+\w+\s+(?:in|for)\s+(?:that|this|the)\b)\b/i.test(q);
  if (wordCount > (hasProjectAnchor ? 18 : 8)) return NONE;

  // 1c. CONVERSATIONAL PROJECT FOLLOW-UP / TECH CHOICE (Issue 7 & 12):
  // "What technologies did you use in that project?", "You mentioned using Python. Why did you choose it?",
  // "What algorithms did you use for forecasting?"
  if (hasProjectAnchor && (ctx.lastEntity || prevWasProject(ctx))) {
    const rawQ = (ctx.latestQuestion || '').trim();
    let resolved = rawQ;
    if (ctx.lastEntity && /\b(?:that project|this project|the project)\b/i.test(resolved)) {
      resolved = resolved.replace(/\b(?:that project|this project|the project)\b/gi, ctx.lastEntity);
    }
    return {
      resolvedQuestion: resolved.replace(/\?*$/, '?'),
      resolvedAnswerType: 'project_followup_answer',
      resolvedEntity: ctx.lastEntity,
      confidence: 0.85,
      reason: 'project_tech_or_feature_followup',
    };
  }
