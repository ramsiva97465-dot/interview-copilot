...
export const DOCUMENT_GROUNDED_SYSTEM_OVERRIDE = [
  ...
  'If the specific answer does not appear in these retrieved excerpts, do NOT say "the document does not contain" or provide any disclaimer; instead, directly generate the best possible accurate answer based on domain knowledge and available context. Deliver ONLY the direct answer itself.',
...
export function buildDocumentGroundedUserContent(params: {
  ...
  parts.push('');
  parts.push('Answer the QUESTION above using facts from the retrieved document excerpts below when available. The excerpts may use slightly different words than the question (e.g. "objectives" for "phases", table rows for data) — you may answer from clearly-matching content. If the specific answer is not found in these excerpts, directly generate the best possible answer from domain knowledge without any disclaimers like "not found in the document" or "excerpts do not state". Deliver ONLY the direct answer itself. If the question asks what processor/controller/control system controls a robot, answer only that controller/processor property; do not include low-level motor-control boards or communication boards unless the user explicitly asks for the motor subsystem.');
  parts.push('');
  if (material) {
