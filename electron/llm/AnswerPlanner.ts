...
Rules:
- Speak directly in the first person ("I led...", "In my previous role...").
- Output ONLY the final answer the candidate should speak. Do NOT output headings or section labels like "Direct Answer:", "Strong Example / STAR:", "Situation:", or "Speakable Final Answer:".
- Never invent metrics, percentages, or achievements not supported by your profile.
- Connect to the role naturally without artificial labels.`;
...
const PROJECT_TEMPLATE = `You are the candidate speaking aloud about your project. Provide a single, fluent, natural first-person answer that directly explains your project in 4-6 speakable sentences.

Rules:
- Speak directly in the first person (e.g. "In my project, [Project Name], I built...").
- Name the project clearly, explain what it does, the technologies used, your personal contribution/role, and the concrete outcome or value.
- Use ONLY grounded project facts from your profile. NEVER invent metrics, percentages, or technologies.
- Output ONLY the words to speak. Do NOT include section titles or labels such as "Best / Relevant Project:", "What I Built:", "Tech Stack:", "My Role:", or "Speakable Final Answer:".`;
...
const JD_FIT_TEMPLATE = `You are the candidate explaining why you are a great fit for this role. Provide a single, cohesive first-person answer (approx. 3-5 sentences).

Rules:
- Speak directly in the first person ("I believe I am well-suited for this role because...").
- Highlight relevant grounded experience, skills, and projects that directly match the role's needs.
- Output ONLY the speakable response. Do NOT use section titles like "Short Fit Summary:", "Matching Experience:", "Why This Role:", or "Speakable Final Answer:".`;
...
const GAP_ANALYSIS_TEMPLATE = `This is a gap or weakness question. Provide a single, confident and honest first-person response (approx. 3-5 sentences).

Rules:
- Speak directly in the first person.
- Acknowledge ONE specific area you have less direct experience with, immediately followed by the adjacent real experience you have and your concrete ramp-up plan.
- Output ONLY the final spoken answer. Do NOT use section titles like "The Honest Gap:", "Why It's Manageable:", "How I'd Close It:", or "Speakable Final Answer:".`;
...
const RESUME_JD_FIT_TEMPLATE = `You are explaining how your background matches this role. Provide a single, polished first-person answer (approx. 3-5 sentences).

Rules:
- Speak in the first person, connecting your real projects and experience to the role's core requirements.
- Never claim JD requirements as experience you do not have.
- Output ONLY the speakable answer. Do NOT use headings or labels like "Short Fit Summary:", "Matching Experience & Projects:", or "Speakable Final Answer:".`;

const RESUME_JD_GAP_TEMPLATE = `This is a gap question about your background versus this job description. Provide a single, candid, and constructive first-person answer (approx. 3-5 sentences).

Rules:
- Lead with an honest acknowledgment of the specific gap between your resume and the JD.
- Follow up immediately with how your adjacent skills and quick-learning capability make it manageable.
- Output ONLY the words to speak. Do NOT use headings like "The Honest Gap:", "Why It's Manageable:", or "Speakable Final Answer:".`;
...
const DOCUMENT_ABSENT_FACT_TEMPLATE = `Answer the question directly and accurately. Use relevant uploaded document evidence if available. If the specific detail is not explicitly mentioned in the documents, generate the best possible answer directly from domain knowledge. Do NOT state "the document does not mention", "not found in the uploaded material", or any disclaimer. Deliver ONLY the direct answer itself.`;
...
const TELL_ME_ABOUT_YOURSELF_TEMPLATE = `You are the candidate answering "Tell me about yourself" / self-introduction.
Provide a concise, polished 4-6 sentence first-person overview of your professional background:
1. Current role / primary identity and professional focus.
2. Key technical strengths and notable projects/work from your experience.
3. What drives you and why you are excited about this conversation/opportunity.
Rules:
- Speak in natural first-person ("I am...", "I specialize in...").
- Ground your details in your real resume / profile.
- Do NOT output section headers, disclaimers, or meta-commentary. Output ONLY the answer to speak.`;

const WHY_HIRE_YOU_TEMPLATE = `You are the candidate answering "Why should we hire you?".
Provide a compelling, confident 4-5 sentence first-person answer highlighting:
1. Direct alignment between your core skills/experience and the role's needs.
2. Proven track record of delivering results and solving key problems in your projects.
3. Fast ramp-up capability, strong work ethic, and enthusiasm to add immediate value.
Rules:
- Speak in first-person ("You should hire me because...", "I bring a strong combination of...").
- Ground in your actual background without inventing fake metrics.
- Do NOT output section headers or disclaimers. Output ONLY the answer to speak.`;

const STRENGTHS_TEMPLATE = `You are the candidate answering "What are your strengths?".
Provide a grounded, articulate 3-5 sentence first-person answer highlighting:
1. Two or three key technical and professional strengths (e.g. problem-solving, clean architecture, rapid learning).
2. A brief, concrete mention of how you demonstrated these in your projects.
Rules:
- Speak in first-person.
- Keep it natural, confident, and professional.
- Do NOT output section headers or disclaimers. Output ONLY the answer to speak.`;

const FIVE_YEARS_TEMPLATE = `You are the candidate answering "Where do you see yourself in five years?".
Provide a thoughtful, ambitious yet realistic 3-4 sentence first-person answer covering:
1. Deepening technical expertise and taking on greater technical ownership or architectural responsibility.
2. Mentoring team members, driving impact on key systems, and growing with the organization.
Rules:
- Speak in first-person.
- Keep it professional, realistic, and positive.
- Do NOT output section headers or disclaimers. Output ONLY the answer to speak.`;

const SCENARIO_TEMPLATE = `You are the candidate answering a situational/scenario interview question (e.g., "What would you do if...", "How do you handle...", "Imagine a situation where...").
Provide a structured, practical, and logical response (3-5 sentences):
1. How you assess and understand the situation or problem.
2. The concrete steps/actions you take to address it effectively and collaborate with stakeholders.
3. How you verify the outcome and learn/prevent future issues.
Rules:
- Answer directly and practically without requiring resume proof or document citation.
- Speak in first-person as a proactive, collaborative professional.
- Output ONLY the spoken answer without headers or disclaimers.`;

const TECH_CHOICE_RATIONALE_TEMPLATE = `You are the candidate explaining why you chose or used a specific technology, tool, or framework in your project (e.g., "Why did you use Python/React/Redis in this project?").
Provide a clear, practical first-person rationale (3-5 sentences):
1. The specific requirements or constraints of the project (e.g., performance, ease of development, ecosystem libraries, data handling).
2. Why this specific technology was the best fit compared to alternatives.
3. How it contributed to the project's success or developer efficiency.
Rules:
- DO NOT just define what the technology is (e.g. do not give a generic "Python is a high-level programming language...").
- Explain the practical rationale and architectural decision for using it in the project.
- Speak in first-person. No section headers.`;
...
const templateFor = (answerType: AnswerType, question?: string): string => {
  const q = (question || '').toLowerCase();

  // Specific HR / Interview questions
  if (/\b(?:tell me about your\s*self|introduce your\s*self|who are you|walk me through your (?:background|resume|profile|journey))\b/i.test(q)) {
    return TELL_ME_ABOUT_YOURSELF_TEMPLATE;
  }
  if (/\b(?:why should (?:we|i|they) hire you|why hire you|why are you (?:the )?(?:best|right) (?:fit|person|candidate))\b/i.test(q)) {
    return WHY_HIRE_YOU_TEMPLATE;
  }
  if (/\b(?:strength|greatest strength|what are your strengths)\b/i.test(q) && !/\bweakness\b/i.test(q)) {
    return STRENGTHS_TEMPLATE;
  }
  if (/\b(?:five years|5 years|in 5 years|where do you see yourself)\b/i.test(q)) {
    return FIVE_YEARS_TEMPLATE;
  }
  // Tech choice rationale in project ("Why did you use Python in your project?", "Why choose React?")
  if (/\bwhy\s+(?:did\s+you|was)\s+(?:use|choose|pick|select|go with|used)\b/i.test(q) || (/\bwhy\b/i.test(q) && /\b(?:in (?:your|the|this) project|for that project)\b/i.test(q))) {
    return TECH_CHOICE_RATIONALE_TEMPLATE;
  }
  // Scenario questions ("What would you do if...", "How do you handle conflict", "Imagine you are...")
  if (/\b(?:what would you do if|how would you handle|how do you handle|imagine (?:you|a situation)|suppose|what if)\b/i.test(q)) {
...
export const planAnswer = (input: PlanAnswerInput): AnswerPlan => {
  ...
  const explicitDocumentModeCodingAsk = /\b(write|implement|code|coding interview|dsa|dry run|time complexity|space complexity|big[-\s]?o|algorithm(?:ic)?|solution code|source code)\b/i.test(text)
    ...
    || /\b(python|javascript|typescript|java|golang|c\+\+|rust|ruby|php|sql|decorators?|fragments?|arrays?|strings?|dicts?|tuples?|lambdas?|generators?|iterators?|gil|concurrency|multithreading|asynchronous|async|await)\b/i.test(text);
  ...
  return {
    ...
    responseTemplate: templateFor(answerType, question),
...
export const formatAnswerPlanForPrompt = (plan: AnswerPlan, includeVerificationSpec = false): string => {
  ...
  const isDocGroundedExempt = plan.forbiddenContextLayers?.includes('reference_files')
    || plan.answerType === 'technical_concept_answer'
    || plan.answerType === 'coding_question_answer'
    || plan.answerType === 'dsa_question_answer'
    || plan.answerType === 'system_design_answer'
    || plan.answerType === 'debugging_question_answer';

  const _docEnforcement = !isDocGroundedExempt && ((plan.docGroundedEnforcementActive
  ...
  const policyLine = _docPolicyActive
    ...
    : (plan.profileContextPolicy === 'required' && !isDocGroundedExempt)
      ...
      : (plan.profileContextPolicy === 'forbidden' || isDocGroundedExempt)
        ? 'Do NOT use or reference the resume, JD, projects, or any personal profile context. Answer directly as an engineer. NEVER state that the resume, document, or uploaded material does not cover this, and NEVER say "From general knowledge".'
