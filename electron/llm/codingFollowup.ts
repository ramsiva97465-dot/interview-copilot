...
// "code only" / "just the code" / "only give code" / "no explanation, just code" or direct coding/SQL requests.
const CODE_ONLY_RE =
  /\b(?:just|only)\s+(?:the\s+|me\s+the\s+)?code\b|\bcode[- ]?only\b|\bonly\s+(?:give|write|show)\s+(?:me\s+)?(?:the\s+)?code\b|\bno\s+explanation,?\s+just\b|\bgive\s+me\s+(?:only\s+)?the\s+code\b|\bcode\s+(?:and\s+)?nothing\s+else\b|\bwrite\s+(?:a\s+|an\s+)?(?:python|javascript|typescript|java|c\+\+|c#|golang|rust|ruby|php|sql|bash|shell)?\s*(?:program|code|script|function|query|solution|implementation)\b|\b(?:sql\s+query|query\s+to|write\s+(?:a\s+)?query)\b/i;
...
export function codingFormatDirective(explicitContract: Exclude<ExplicitCodingContract, null>): string {
  const body = (() => {
    switch (explicitContract) {
      case 'code_only':
        return `The user asked for CODE/QUERY ONLY. Output ONLY the working code or query in a single fenced code block with a language tag (e.g. \`\`\`python or \`\`\`sql). Absolutely NO introductory or closing prose, NO "## Approach" / "## Complexity" / "## Dry Run" headings, NO explanation, NO dry run, NO test cases. Output strictly the code only.`;
