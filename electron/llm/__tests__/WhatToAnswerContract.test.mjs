...
// must FLAG (so the live path repairs) a MeetFloo-identity leak, a false refusal
...
describe('Issue 2: ProfileOutputValidator flags WTA contract violations', () => {
  ...
  test('"I am MeetFloo, an AI assistant." → assistant_identity_leak', () => {
    assert.ok(errs('I am MeetFloo, an AI assistant.').includes('assistant_identity_leak'));
  });
  ...
  test('a legitimate job title "I\'m an AI Engineer" is NOT flagged as a MeetFloo leak', () => {
