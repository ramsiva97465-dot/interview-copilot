...
- **MeetFloo API Promo Toaster redesign** — Iridescent animated border (Aurora UI, 6 s loop), breathing glow, SVG grain texture, gradient headlines, and `useReducedMotion` gating for accessibility
...
- **Profile Intelligence settings tab** — Reordered navigation so Profile Intelligence appears immediately after MeetFloo API
...
- **Native build hardening** — `build:native` added to `app:build` pipeline; macOS defaults to current-arch only (`MEETFLOO_BUILD_ALL_MAC_ARCHES=1` for dual build); `verifyArtifacts()` throws on missing `.node` files (fail-fast)
...
- **For .dmg downloads:**
  1. Open Terminal and run:
     ```bash
     xattr -cr ~/Downloads/MeetFloo-2.2.0-arm64.dmg
     # Or for Intel Macs:
     xattr -cr ~/Downloads/MeetFloo-2.2.0-x64.dmg
     ```
