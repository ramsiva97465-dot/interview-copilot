...
import { splitGistLine, splitGistLineStreaming, collapseBlockGaps, stripReasoningArtifacts } from '../lib/displayMarkup';
...
const NativelyInterface: React.FC<NativelyInterfaceProps> = ({
  ...
  const lastAutoAnswerQuestionRef = useRef<string | null>(null);
    ...
    window.electronAPI
      ...
      .then((res: any) => {
        if (res?.adjusted && res.effectiveModel) {
          setCurrentModel(res.effectiveModel);
        }
      })
  ...
  const paintRevealedNow = useCallback((nowMs?: number) => {
    ...
    const cleanedRevealed = stripReasoningArtifacts(revealed);
    const { body: revealedBody, gist: revealedGist } = splitGistLineStreaming(cleanedRevealed);
  ...
  useEffect(() => {
    ...
    cleanups.push(
      window.electronAPI.onNativeAudioTranscript((transcript) => {
        ...
        const trimmedInterviewerText = (transcript.text || '').trim();
        if (trimmedInterviewerText.length > 5) {
          setMessages((prev) => {
            const alreadyPresent = prev.some(
              (m) => (m.role === 'interviewer' || m.role === 'user') && m.text.trim().toLowerCase() === trimmedInterviewerText.toLowerCase()
            );
    ...
    cleanups.push(
      window.electronAPI.onIntelligenceSuggestedAnswerToken((data) => {
        ...
        if (data.question && data.question.trim()) {
          ...
          if (lastAutoAnswerQuestionRef.current !== qText) {
            lastAutoAnswerQuestionRef.current = qText;
            setMessages((prev) => {
              const last = prev[prev.length - 1];
              if (last && (last.role === 'interviewer' || last.role === 'user') && last.text === qText) {
                return prev;
              }
              return [
                ...prev,
    ...
    cleanups.push(
      window.electronAPI.onIntelligenceSuggestedAnswer((data) => {
        ...
        if (data.question && data.question.trim()) {
          const qText = data.question.trim();
          setMessages((prev) => {
            const hasQ = prev.some((m) => (m.role === 'interviewer' || m.role === 'user') && m.text === qText);
            if (hasQ) return prev;
            return [
              ...prev,
              {
        ...
        lastAutoAnswerQuestionRef.current = null;
  ...
  const handleAnswerNow = async () => {
    if (isManualRecording) {
      ...
      try {
        ...
        const directTranscriptFallback = !voiceInputRef.current?.trim() && !manualTranscriptRef.current?.trim() && currentAttachments.length === 0
          ? (pendingRollingPartialRef.current ? mergeRollingTranscriptPartial(rollingTranscript, pendingRollingPartialRef.current) : rollingTranscript)?.split('  ·  ').slice(-3).join('  ·  ').trim()
          : '';

        const question = mergeTranscriptChunks(
          voiceInputRef.current,
          manualTranscriptRef.current,
        ).trim() || directTranscriptFallback;
        setVoiceInput('');
        voiceInputRef.current = '';
        setManualTranscript('');
        manualTranscriptRef.current = '';
          ...
          if (sttUserStatus === 'failed' && sttUserError) {
            ...
            setMessages((prev) => [
              ...prev,
              {
                id: genMessageId(),
                role: 'system',
                text: '⚠️ No speech detected. Try speaking closer to your microphone or verify meeting audio.',
              },
            ]);
          }
          return;
...
{showRestartInstead ? (
              ...
              {showAnswerPanel && (
                <motion.div
                  ...
                  className={`relative flex-1 overflow-y-auto overflow-x-hidden ${hasChatContent ? 'p-4 space-y-3' : 'p-0'} no-drag isolate answer-scrollbar`}
                  layout={false}
                  style={{ maxHeight: scrollMaxH, minHeight: scrollMinH }}
                >
