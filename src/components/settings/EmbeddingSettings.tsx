...
import { AlertCircle, Check, ChevronDown, Cloud, Download, ExternalLink, HardDrive, KeyRound, Loader2, Monitor, Search, Server, Sparkles, Trash2, X } from 'lucide-react';
...
export const EmbeddingSettings: React.FC<EmbeddingSettingsProps> = ({ renderParts }) => {
    ...
    // Local HuggingFace embedding model download states
    ...
    useEffect(() => {
        ...
        window.electronAPI.localEmbeddingGetDownloadState?.().then((states: any[]) => {
            if (Array.isArray(states)) {
                const progressMap: Record<string, number> = {};
                const downloading = new Set<string>();
                for (const st of states) {
                    if (st.status === 'downloading' || st.status === 'verifying') {
                        downloading.add(st.modelId);
                        progressMap[st.modelId] = st.progress || 0;
                    }
                }
        ...
        const unsub = window.electronAPI.onLocalEmbeddingDownloadState?.((data) => {
            ...
            if (status === 'downloading' || status === 'verifying') {
                setDownloadingSet(prev => new Set(prev).add(modelId));
                ...
                setDownloadErrors(prev => ({ ...prev, [modelId]: error || t('Download failed') }));
    ...
    const handleStartDownload = useCallback(async (modelId: string) => {
        setDownloadingSet(prev => new Set(prev).add(modelId));
        ...
        const res = await window.electronAPI.localEmbeddingStartDownload?.(modelId);
        if (res && !res.success && res.error) {
            ...
            setDownloadErrors(prev => ({ ...prev, [modelId]: res.error || t('Failed to start download') }));
        }
    }, [t]);

    const handleCancelDownload = useCallback(async (modelId: string) => {
        await window.electronAPI.localEmbeddingCancelDownload?.(modelId);
        setDownloadingSet(prev => {
            const next = new Set(prev);
            next.delete(modelId);
            return next;
    ...
    const handleDeleteModel = useCallback(async (modelId: string) => {
        await window.electronAPI.localEmbeddingDeleteModel?.(modelId);
        await refresh();
    }, [refresh]);
        ...
        return (
            <div key={p.id} className="aip-card aip-provider" data-off={p.available ? undefined : 'true'}>
                ...
                <div className="aip-provider-head">
                    ...
                    <h4 className="aip-card-title truncate min-w-0">{p.id === 'local' ? t('Local Embeddings') : p.name}</h4>
                    {p.id === 'local' ? (
                        <AipBadge tone="ok" label={t('On-Device')} />
                    ) : (
                ...
                {p.id === 'local' ? (
                    (() => {
                        ...
                        const bundledModel = localModels.find(m => !m.downloadable);
                        const downloadableModels = localModels.filter(m => m.downloadable);
                        ...
                        const filteredOnnxModels = downloadableModels.filter(m => {
                            ...
                            if (localModelQuery.trim()) {
                                const q = localModelQuery.trim().toLowerCase();
                                const matchesName = (m.label || m.id).toLowerCase().includes(q);
                                const matchesNote = (m.note || '').toLowerCase().includes(q);
                                const matchesDims = `${m.dimensions}`.includes(q);
                                const matchesSize = `${m.sizeMB ?? m.sizeMb ?? ''}`.includes(q);
                                return matchesName || matchesNote || matchesDims || matchesSize;
                            }
                            return true;
                        });

                        const showBundled = (localFilterTab === 'all' || localFilterTab === 'installed') && (
                            !localModelQuery.trim() || (
                                !!bundledModel && (
                                    (bundledModel.label || bundledModel.id).toLowerCase().includes(localModelQuery.trim().toLowerCase()) ||
                                    (bundledModel.note || '').toLowerCase().includes(localModelQuery.trim().toLowerCase())
                                )
                            )
                        );
                            ...
                            <div className="space-y-3 pt-1">
                                <div className="flex items-center justify-between gap-3 flex-wrap text-[10px] aip-muted px-1">
                                    <p className="min-w-0 flex-1">
                                        <strong className="text-white/80">{t('Embedding (Search Index): ')}</strong>
                                        {t('Converts your meetings, transcripts, and notes into searchable vectors for fast semantic search. 100% on-device & private.')}
                                    </p>
                                ...
                                <div className="flex items-center justify-between gap-3 flex-wrap pt-1">
                                    <p className="text-[11px] px-1" style={{ color: 'var(--aip-tertiary)' }}>
                                        {t('Best for this')} {isMac ? 'Mac' : 'PC'}: <span className="font-medium" style={{ color: 'var(--aip-secondary)' }}>BGE Large v1.5</span>
                                    </p>
                                    ...
                                    <div className="flex items-center gap-1 bg-white/5 p-0.5 rounded-md text-[11px]">
                                        <button
                                            ...
                                            style={{
                                                background: localFilterTab === 'all' ? 'rgba(255, 255, 255, 0.1)' : 'transparent',
                                                fontWeight: localFilterTab === 'all' ? 600 : 400,
                                            }}
                                        ...
                                        <button
                                            ...
                                            className="px-2 py-0.5 rounded text-white transition-colors"
                                            style={{
                                                background: localFilterTab === 'installed' ? 'rgba(255, 255, 255, 0.1)' : 'transparent',
                                                fontWeight: localFilterTab === 'installed' ? 600 : 400,
                                            }}
                                        ...
                                        <button
                                            ...
                                            className="px-2 py-0.5 rounded text-white transition-colors"
                                            style={{
                                                background: localFilterTab === 'recommended' ? 'rgba(255, 255, 255, 0.1)' : 'transparent',
                                                fontWeight: localFilterTab === 'recommended' ? 600 : 400,
                                            }}
                                ...
                                <div className="aip-well aip-scroll-y p-2.5 space-y-3.5" style={{ maxHeight: 380 }}>
                                    ...
                                    {showBundled && bundledModel && (
                                        <section className="space-y-1.5">
                                            ...
                                            <div
                                                ...
                                                data-active={isMiniLMActive ? 'true' : undefined}
                                            >
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex items-center gap-2 min-w-0 flex-1 flex-wrap sm:flex-nowrap">
                                                        <span
                                                            ...
                                                            style={{ background: isMiniLMActive ? 'var(--aip-accent)' : 'var(--aip-tertiary)' }}
                                                        />
                                                        <span className="text-xs font-semibold text-white truncate">{bundledModel.label || bundledModel.id}</span>
                                                        <AipBadge tone="neutral" label={t('Built-in Default')} />
                                                        {isMiniLMActive && <AipBadge tone="ok" label={t('In use')} />}
                                                    </div>
                                                    <div className="shrink-0">
                                                        {!isMiniLMActive && (
                                                            <button
                                                                ...
                                                                data-size="sm"
                                                                disabled={!!pending}
                                                                onClick={() => void select('local', bundledModel.id, bundledModel.dimensions)}
                                                            >
                                                                <span>{t('Use')}</span>
                                                            </button>
                                                        )}
                                                ...
                                                <div className="text-[10.5px] aip-muted pl-3.5 flex items-center gap-2 flex-wrap">
                                                    <span>{bundledModel.dimensions} dims · ~{bundledModel.sizeMB ?? bundledModel.sizeMb ?? 23} MB · Apache-2.0</span>
                                                </div>
                                    ...
                                    {/* Section 2: Hugging Face ONNX Models */}
                                        ...
                                        <section className="space-y-1.5">
                                            <header className="flex items-baseline justify-between gap-3 px-1">
                                                ...
                                                <span className="text-[10px] tabular-nums" style={{ color: 'var(--aip-tertiary)' }}>
                                                    {downloadedOnnxCount}/{downloadableModels.length}
                                                </span>
                                            </header>
                                            <div className="space-y-1.5">
                                                {filteredOnnxModels.map(m => {
                                                        ...
                                                        >
                                                            <div className="flex items-center justify-between gap-3">
                                                                <div className="flex items-center gap-2 min-w-0 flex-1 flex-wrap sm:flex-nowrap">
                                                                    <span
                                                                        ...
                                                                        className="w-1.5 h-1.5 rounded-full shrink-0"
                                                                        style={{
                                                                            background: isCurrentActive ? 'var(--aip-accent)' : isDownloaded ? 'var(--aip-tertiary)' : 'transparent',
                                                                            border: isDownloaded || isCurrentActive ? undefined : '1px solid var(--aip-border-strong)',
                                                                        }}
                                                                    />
                                                                    <span className="text-xs font-semibold text-white truncate">{m.label || m.id}</span>
                                                                    {m.id === 'Xenova/bge-large-en-v1.5' && (
                                                                        <AipBadge tone="info" label={t('Recommended · Max Accuracy')} />
                                                                    )}
                                                                    {m.id === 'Xenova/bge-base-en-v1.5' && (
                                                                        <AipBadge tone="info" label={t('Recommended · Balanced')} />
                                                                    )}
                                                                    {m.recommended && m.id !== 'Xenova/bge-large-en-v1.5' && m.id !== 'Xenova/bge-base-en-v1.5' && (
                                                                        <AipBadge tone="info" label={t('Recommended')} />
                                                                    )}
                                                                    {isCurrentActive && <AipBadge tone="ok" label={t('In use')} />}
                                                                    ...
                                                                    {!isDownloaded && (
                                                                        isDownloading ? (
                                                                            <button
                                                                                ...
                                                                                className="aip-btn"
                                                                                data-size="sm"
                                                                                onClick={() => void handleCancelDownload(m.id)}
                                                                            >
                                                                                <X size={12} strokeWidth={1.75} aria-hidden="true" />
                                                                                <span>{t('Cancel')}</span>
                                                                            </button>
                                                                            ...
                                                                            <button
                                                                                ...
                                                                                className="aip-btn"
                                                                                data-size="sm"
                                                                                disabled={downloadingSet.size > 0}
                                                                                onClick={() => void handleStartDownload(m.id)}
                                                                            >
                                                                    ...
                                                                    {isDownloaded && !isCurrentActive && (
                                                                        <>
                                                                            <button
                                                                                ...
                                                                                data-size="sm"
                                                                                disabled={!!pending}
                                                                                onClick={() => void select('local', m.id, m.dimensions)}
                                                                            >
                                                                                <span>{t('Use')}</span>
                                                                            </button>
                                                                            <button
                                                                                ...
                                                                                title={t('Delete downloaded model')}
                                                                            >
                                                            ...
                                                            <div className="text-[10.5px] aip-muted pl-3.5 flex items-center gap-2 flex-wrap">
                                                                <span>
                                                                    {[
                                                                        `${m.dimensions} dims`,
                                                                        modelSize ? `~${modelSize} MB` : null,
                                                                        'Apache-2.0',
                                                                        m.id === 'nomic-ai/nomic-embed-text-v1' ? '8k context' : null,
                                                                    ].filter(Boolean).join(' · ')}
                                                                </span>
                                                            </div>
                                                            ...
                                                            {isDownloading && (
                                                                <div className="space-y-1 pl-3.5 pt-1">
                                                                    ...
                                                                    <div className="text-[10px] aip-muted flex justify-between">
                                                                        <span>{`${Math.round(progress)}% · Downloading from HuggingFace`}</span>
                                                                        <span>{`${Math.round((progress * (modelSize ?? 100)) / 100)} / ${modelSize ?? 100} MB`}</span>
                                                                    </div>
                                                                </div>
                                                            )}
                                                            ...
                                                            {downloadError && (
                                                                <div className="aip-inline-warn flex items-start gap-2 ml-3.5 mt-1" role="status">
                                                                    ...
                                                                    <span className="min-w-0 text-xs aip-danger-fg">{downloadError}</span>
                                                                </div>
                                                            )}
                                                        </div>
                                                    );
                                    ...
                                    {filteredOnnxModels.length === 0 && !showBundled && (
