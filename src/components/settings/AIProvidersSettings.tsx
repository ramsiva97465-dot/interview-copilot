...
export const AIProvidersSettings: React.FC<AIProvidersSettingsProps> = ({
    ...
    const keyFields: Record<CloudProviderId, [string, (v: string) => void]> = {
        sarvam: [sarvamApiKey, setSarvamApiKey],
    ...
    useEffect(() => {
        const loadCredentials = async () => {
            try {
                ...
                if (creds) {
                    setHasStoredKey({
                        sarvam: (creds as any).hasSarvamKey || false,
    ...
    const handleSetAntigravityDefault = async (modelId: string) => {
        ...
        try {
            ...
            const res = await window.electronAPI?.setDefaultModel(modelId);
    ...
    const handleSaveKey = async (provider: string, key: string, setter: (val: string) => void) => {
        ...
        try {
            let result;
            // @ts-ignore
            if (provider === 'sarvam') result = await window.electronAPI.setSarvamApiKey(key);
            // @ts-ignore
    ...
    const performRemoveKey = async (provider: string, setter: (val: string) => void) => {
        ...
        try {
            let result;
            // @ts-ignore
            if (provider === 'sarvam') result = await window.electronAPI.setSarvamApiKey('');
            // @ts-ignore
