...
// Who the OTHER party is, per mode. MeetFloo's asymmetry: in recruiting the
// user is the interviewer, so the system channel is the candidate.
const OTHER = {
  'technical-interview': 'INTERVIEWER', 'looking-for-work': 'INTERVIEWER', 'sales': 'PROSPECT',
...
for (const [mode, other] of Object.entries(OTHER)) {
  ...
  const turns = outLines.join('\n').split('\n').map(l => l.trim()).filter(l => re.test(l))
    .map(l => {
      ...
      return s.split(' ').length >= 3 ? `${w.toUpperCase()}: ${s}` : null;
    }).filter(Boolean);
