/**
 * Same question, but against the ACTUAL MeetFloo backend rather than raw
 * provider APIs: POST ${MEETFLOO_API_URL}/v1/chat with x-MeetFloo-key, SSE.
 * Shape lifted from LLMHelper.streamWithMeetFloo (endpoint :7185, body :3575).
 *
...
const KEY = k('MEETFLOO_API_KEY');
const BASE = (process.env.MEETFLOO_API_URL || 'https://api.MeetFloo.software').replace(/\/+$/, '');
const GATE = 160;
// F-301: the live path gives the MeetFloo cascade route 13s, not 7s.
...
async function stream(userMessage) {
  ...
  const res = await fetch(`${BASE}/v1/chat`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Accept': 'text/event-stream', 'x-MeetFloo-key': KEY, 'X-Request-Id': `probe_${Date.now()}` },
