/**
 * REAL APP + REAL MEETFLOO BACKEND.
 *
 * Drives the shipped what-to-answer path (the one carrying the 160-char
 * "useful" gate) with `MeetFloo` as the AI provider, and checks whether a
...
const setup = await RAW(async () => {
  ...
  const out = {};
  try { const c = await api.modesCreate({ name: 'MeetFlooShortProbe', templateType: 'general' }); const id = c?.mode?.id ?? c?.id; if (id) { await api.modesSetActive(id); out.mode = id; } } catch (e) { out.modeErr = String(e).slice(0, 90); }
  try { out.setModel = await api.setModel('MeetFloo'); } catch (e) { out.setModelErr = String(e).slice(0, 120); }
  return out;
});
...
console.log(canned > 0
  ? '\nRESULT: a correct answer WAS replaced on the real MeetFloo provider.'
  : '\nRESULT: no replacement observed on the real MeetFloo provider.');
await app.close();
